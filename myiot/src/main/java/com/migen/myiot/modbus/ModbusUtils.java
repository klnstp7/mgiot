...
public class ModbusUtils {
     ...
     * @return
     */
    public static byte[] getDataWithCrc(byte[] bytes) {
        byte[] crc = intToBytes(calcCrc16(bytes));
        byte[] result = new byte[bytes.length+crc.length];
        System.arraycopy(bytes,0,result,0,bytes.length);
        System.arraycopy(crc,0,result,bytes.length,crc.length);
        return result;
    }
     ...
     * @return
     */
    public static byte[] cmdToBytes(Command command){
        byte[] tempfront={command.getSlaveId(),command.getFunctionCode()};
        byte[] tempstart=BytesUtil.shortToByte(command.getStart());
        byte[] temp=ArrayUtils.addAll(tempfront,tempstart);
        byte[] temp1=ArrayUtils.addAll(temp,command.getDataSegment());
        byte[] result =getDataWithCrc(temp1);
        return result;
    }
